...
__constant__ float d_kernel_edge[FILTER_SIZE][FILTER_SIZE] = {
    {0.0f, -1.0f, 0.0f},
    {-1.0f, 4.0f, -1.0f},
    {0.0f, -1.0f, 0.0f}
};

__global__ void processImage(unsigned char *input, unsigned char *output, unsigned int pitch, unsigned int width, unsigned int height) {
    ...
    __shared__ unsigned char sBuffer[BLOCK_SIZE + FILTER_SIZE - 1][BLOCK_SIZE + FILTER_SIZE - 1];
    ...
    if (threadIdx.x < TILE_SIZE && threadIdx.y < TILE_SIZE) {
        for (int r = 0; r < FILTER_SIZE; r++) {
            for (int c = 0; c < FILTER_SIZE; c++) {
                int x_offset = threadIdx.x + c;
                int y_offset = threadIdx.y + r;

                if (x_offset < BLOCK_SIZE + FILTER_SIZE - 1 && y_offset < BLOCK_SIZE + FILTER_SIZE - 1) {
                    sum += sBuffer[y_offset][x_offset] * d_kernel_edge[r][c];                    
                }
        ...
        if(x_o < width && y_o < height) {
            output[y_o * width + x_o] = static_cast<unsigned char>(sum);
        }
    }
}
...
int main(){
    ...
    processImage<<<gridSize, blockSize>>>(d_r, d_r_n, pitch_r, width, height);
    processImage<<<gridSize, blockSize>>>(d_g, d_g_n, pitch_g, width, height);
    processImage<<<gridSize, blockSize>>>(d_b, d_b_n, pitch_b, width, height);
